...
class Bunch_Enqueue
{
	...
	function bunch_enqueue_scripts()
	{
		...
		$header_style = charitypro_set( $options, 'header_style' );
		
 
		$version = $current_theme->get( 'Version' );
		
		$dark_color = ( charitypro_set( $options, 'website_theme' ) == 'dark' ) ? true : false;
		
		$dark_color = ( charitypro_set( $_GET, 'color_style' ) == 'dark' ) ? true : $dark_color;
		
		if(charitypro_set($options, 'color_scheme') == 'color2') : $color = 'css/color2.css'; else: $color = 'css/color1.css'; endif;	
		...
		if( is_single() ) {
			...
			if( $format == 'gallery' ) wp_enqueue_script( array( 'charitypro_jquery-flexslider' ) );
			if( $format == 'video' || $format == 'audio' ) wp_enqueue_script( array( 'charitypro_jquery-fitvids' ) );
		}
		if( is_singular( 'bunch_projects' ) || $wp_query->is_posts_page || is_search() || is_tag() || is_category() || is_author() || is_archive() ) 
  		wp_enqueue_script( array('charitypro_jquery-flexslider', 'charitypro_owl.carousel', 'charitypro_jquery-fitvids') );
		wp_enqueue_script( array('charitypro_custom_script') );
		
		wp_add_inline_style( 'charitypro-main-style', $this->wp_head() );
		
		
	}
	
	function charitypro_get_font_settings( $styles )
	{
		$opt = _WSH()->option();
		$protocol = ( is_ssl() ) ? 'https' : 'http';
		$font = array();
		...
		if( charitypro_set( $opt, 'use_custom_font' ) ){
			...
			if( $h1 = charitypro_set( $opt, 'h1_font_family' ) ) $font[$h1] = urlencode( $h1 ).':400,300,600,700,800';
			if( $h2 = charitypro_set( $opt, 'h2_font_family' ) ) $font[$h2] = urlencode( $h2 ).':400,300,600,700,800';
			if( $h3 = charitypro_set( $opt, 'h3_font_family' ) ) $font[$h3] = urlencode( $h3 ).':400,300,600,700,800';
		}
		
		if( charitypro_set( $opt, 'body_custom_font' ) ){
			if( $body = charitypro_set( $opt, 'body_font_family' ) ) $font[$body] = urlencode( $body ).':400,300,600,700,800';
		}
	...
	function wp_head()
	{
		...
		wp_add_inline_script( 'charitypro-main-script', $h_js );
			$style = '';
		if ( charitypro_set( $opt, 'body_custom_font' ) ) {
		...
		$style .= charitypro_set( $opt, 'header_css' );
