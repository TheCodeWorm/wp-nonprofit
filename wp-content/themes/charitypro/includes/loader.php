...
add_action( 'init', 'charitypro_bunch_theme_init');


if( !function_exists( 'charitypro_set' ) ) {
	function charitypro_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('charitypro_printr' ) ) {
	function charitypro_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('charitypro__font_awesome' ) ) {
	function charitypro_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = charitypro_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('charitypro_load_class' ) ) {
	function charitypro_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
charitypro_load_class( 'enqueue', 'helpers', false );
charitypro_load_class( 'bootstrap_walker', 'helpers', false );


if( charitypro_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function charitypro_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include CHARITYPRO_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include CHARITYPRO_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = CHARITYPRO_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = CHARITYPRO_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( charitypro_set( $_GET, 'page' ) == 'charitypro'.'_option' ) 
		{
			if( charitypro_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'charitypro_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = CHARITYPRO_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'charitypro'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'charitypro'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'charitypro' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'charitypro' ), // menu label
	));
